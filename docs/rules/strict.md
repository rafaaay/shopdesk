...
This rule disallows strict mode directives, no matter which option is specified, if configuration specifies either of the following as [parser options](/docs/user-guide/configuring.md#specifying-parser-options):
...
* `"safe"` (default) corresponds either of the following options:
    * `"global"` if considers a file to be a **CommonJS** module
    * `"function"` otherwise
...
The `"safe"` option corresponds to the `"global"` option if considers a file to be a **Node.js** or **CommonJS** module because the configuration specifies either of the following:
...
(removed) The default option (that is, no string option specified) for this rule was **removed** in v1.0. The `"function"` option is most similar to the removed option.
