...
Imports are an ES6/ES2015 standard for making the functionality of other modules available in your current module. In CommonJS this is implemented through the `require()` call which makes this rule roughly equivalent to its CommonJS counterpart `no-restricted-modules`.
...
```js
/*no-restricted-imports: ["error", { "paths": ["fs"], "patterns": ["eslint/*"] }]*/
...
```js
/*no-restricted-imports: ["error", { paths: [{ name: "foo", importNames: ["DisallowedObject"] }] }]*/
...
Don't use this rule or don't include a module in the list for this rule if you want to be able to import a module in your project without an error or warning.
